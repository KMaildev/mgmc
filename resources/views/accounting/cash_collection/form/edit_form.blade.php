<form action="{{ route('cash_collection.update', $cash_collections_data->id) }}" method="POST" autocomplete="off"
    id="edit-form">
    @csrf
    @method('PUT')
    <tr>
        ...
        <td>
            <input type="text"
                ...
                value="{{ $cash_collections_data->cash_collection_date }}" />
        ...
        <td>
            <input type="text"
                ...
                name="sale_discount_debited" style="width: 100%"
                value="{{ $cash_collections_data->sale_discount_debited }}" />
...
@section('script')
    {!! JsValidator::formRequest('App\Http\Requests\UpdateCashCollection', '#edit-form') !!}
    <script>
        $(document).ready(function() {
            ...
            var customerID = {{ $cash_collections_data->customer_id }};
            var SalesInvoicesID = {{ $cash_collections_data->sales_invoice_id }};
            var SaleJournalId = {{ $cash_collections_data->sales_journal_id }};

            get_customer_ajax(customerID);
            get_sales_items_ajax(SalesInvoicesID);
            get_sales_journals_ajax(SaleJournalId)
